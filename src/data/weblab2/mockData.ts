...
const sharedAttachments: ChatAttachment[] = [
  {
...
export const fileChipActionPrefilledAttachments = sharedAttachments.map((a) => a.path);
...
export const tutorActionCardPrefilledAttachments = sharedAttachments
  .filter((a) => a.imageSrc)
  .map((a) => a.path);
...
export const sharedAttachmentMeta: Record<string, ChatAttachment> = Object.fromEntries(
  sharedAttachments.map((a) => [a.path, a]),
);

export { sharedAttachments };

...
Web Lab 2 validation experiments can opt into a Tutor review card through a route-provided validation review config. This adds an explicit **Check my work** affordance and in-chat review offers for likely review/continue intent, while the review itself uses cheap local evidence from current project files, configured checks, changed files, and chat history. The validation progression routes demonstrate technical, open-ended, and hybrid review contexts without adding those demos to the standard Web Lab 2 example list.

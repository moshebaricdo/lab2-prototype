...
- `useBackpackState` / `BackpackProvider` for cross-level Backpack persistence (`localStorage` key `lab2:backpack`). `Lab2Shell` wraps the resource panel and workspace in `BackpackProvider` so file-manager save actions and the Backpack tab share one store. IDE routes pass `backpackImportLab` and `onImportBackpackItem` into `Sidebar`; per-lab extension allow-lists in `backpackImportAllowlist.ts` gate the **+** import action (unsupported types stay visible with a disabled button and tooltip). Optional `backpackFilterExperiment` on experiment routes switches panel layout (`default` source-lab sections vs filter pills, supported toggle, or dropdown — see Backpack Filtering sample progression). A session **Added** badge appears on that chip after a successful import.
...
Sketch Lab lives under `components/ide/sketchlab/views` on a customized `@xyflow/react` canvas. `SketchLabLevelPage.tsx` composes `Lab2Shell` with Instructions + mock AI Tutor tabs (Version History hidden) and passes canvas state through `useSketchLabState` with route-scoped storage keys. The workspace header exposes download (JSON export) and start-over actions; node palette and property panel float over the canvas. Backpack tab is available via the shared `BackpackProvider` in `Lab2Shell`, but canvas → Backpack save/import is not wired yet — types and allow-lists already include `sketch-lab`. See `src/guidelines/level-types/sketchlab.md`.

...
- `instructionOpeningRunner.ts` requests an LLM-authored Tutor-primary opening from the canonical Markdown plus `InstructionGuide` when a session API key is present. The system prompt asks for finished, succinct, friendly, ready-to-display sentences and forbids scaffolding phrases like "This level is about"/"Aim for a page where". The runner shows those goal/success/first-move sentences verbatim (light whitespace/punctuation normalization only, never re-wrapped in the programmatic tone templates) and returns per-step `shortLabel`/`summary` rows keyed to the guide's step ids. On missing keys, incomplete model output, or request failure, the app falls back to `tutorOpening.ts`.
...
- Web Lab routes can opt into Tutor-primary instruction delivery with `tutorInstructionsDelivery`. The route still treats `instructions.md` / `instructionsMarkdown` as the authoritative student-facing fallback, but derives either a linear or choice-based `InstructionGuide` plus a separate `TutorOpening`. On panel mount, `AiTutorPanel` shows a brief "Preparing instructions" thinking state, calls `runInstructionOpening()` when a key is present, and commits the opening as message `0` keyed by `instructionGuideSignature`; without a key or on runner failure it commits the programmatic fallback immediately. If the student sends before the opening resolves, the async seed is cancelled. The collapsed instructions drawer can show a pinned current-step summary derived from guide state and the runner's step summaries; expanding still shows the full Markdown. Subsequent typed conversation passes the current `instructionFocus` into normal guidance. Student typed conversation drives progression; the guide does not expose step/focus chips or return canned chip replies. Seeded guide messages are reproducible from the current guide/opening and are not the source of truth.
...
- Change Tutor-primary LLM opening behavior in `instructionOpeningRunner.ts` and `openAiProvider.ts` (`requestInstructionOpening`), programmatic fallback copy in `tutorOpening.ts`, pinned-step copy in `instructionPinnedStep.ts`, instruction guide derivation in `instructionGuide.ts`, and deterministic guide-state behavior in `instructionCoach.ts`. Keep intended instruction flow separate from `levelProgress`; delivering or advancing a guide step must not mark validation criteria complete.

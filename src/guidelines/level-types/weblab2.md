...
- `src/pages/weblab2/WebLab2ValidationProgressionLevel.tsx`
...
- Functional AI Tutor prototype can respond with no-edit guidance, Markdown planning proposals, or validated project edit proposals when a session API key is present. Route-level Tutor proposal orchestration lives in `useWebLab2TutorFlow`, while `WebLab2LevelPage` stays focused on composing the Lab2 shell, resource panel, workspace, and modals.
...
- Web Lab 2 can enable an intentional Tutor review affordance for validation experiments. When a route passes a validation review config, the Tutor panel shows **Check my work** and can surface an in-chat review offer when the student says things like “check this” or “can I continue?”. The first iteration uses cheap local evidence from current project files, configured checks, changed files, and chat history; it does not continuously call an AI grader in the background.
...
- Validation review experiment fixtures live under `src/data/weblab2/projects/validation-*`. The original technical bug-fix fixture powers `/levels/weblab2-validation-test` and the first validation progression level; the two additional progression fixtures cover open-ended creation and hybrid refinement contexts.

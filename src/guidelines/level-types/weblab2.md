...
- `/levels/progression-backpack-labs` → redirects to `progression-backpack-labs-web` (cross-lab backpack: Web → Python → Sketch → AI Chat)
...
- `src/components/ide/shared/CreateFileModal.tsx`
- `src/components/ide/weblab2/views/NameInputModal.tsx`
...
- Lab2 `TopNavigation` follows CADS Global Header **labLevel**: 48px brand bar (always dark / on-brand, independent of the page theme toggle), extraSmall outlined Username + extraSmall text Help/Menu, lesson title + 30px progress indicator locked to light mode (numbered active bubble, assessment stars, more-levels chevron). Continue defaults to the sidebar footer; when `continueButtonPlacement="header"`, it is contained primary to the right of the progress indicator.
...
- The **Backpack Across Labs** progression (`BackpackCrossLabProgressionPages.tsx`, `/levels/progression-backpack-labs-*`) walks Web Lab → Python → Sketch → AI Chat with the same pre-seeded file-type mix (`src/data/backpack/crossLabBackpackSeed.ts`). Seed items are merged by id (`backpackEnsureSeedItems`) so they appear even when the backpack already has files. Each level opens on the Backpack tab so engineers can compare allow-lists, the type dropdown, and **Not supported in this lab**.
- Tabbed resource panel integration (AI Tutor, Version History, Backpack; optional tabs supported)
- **Backpack** — shared cross-level file store persisted in `localStorage` (`lab2:backpack`). The Backpack rail tab sits after Version History. Students can save project files from the file manager context menu (**Save to Backpack**) or header menu (**Import from Backpack** opens the panel). Saved items render as file chips with add-to-project (**+** tooltip: "Add to project"), download, rename, and delete actions. Per-lab import allow-lists (`backpackImportAllowlist.ts`) disable **+** with a “Not supported in this lab” tooltip when the file type does not match the active lab (e.g. `.py` in Web Lab, `.html` in Python Lab, non-images in Sketch Lab). AI Chat Lab currently allows all file types. Production panel layout (`backpackFilterExperiment` default **type-availability**): a 50/50 pair of labelled small dropdowns that shrink with the panel (long trigger labels ellipsize) — **File type** (extension options derived from backpack contents, file-type start icons) and **Sort** (text-only: Alphabetical A–Z / Z–A, Newest first, Oldest first, File type); lab-supported files stay on top and unsupported files sink under a collapsible **Not supported in this lab (N)** divider (xs gray tertiary button with trailing chevron, collapsed by default — same pattern as version-history autosaves). After a successful import, the chip **+** is replaced by a CADS `Tag` (`size="small"`, `color="success"`, start check). Deleting a backpack item first opens a CADS `Dialog` (`type="iconTop"`) to confirm, then an error `Toast` with **Undo**. Saving a project file to the backpack shows a CADS `Toast` at **topCenter** of the viewport (success with **Undo**, or error). Web Lab and Python Lab routes wire `onImportBackpackItem` so the panel **+** button copies backpack file content into the active project tree.
...
- The shared AI Tutor stream uses CADS `AiChatMessage` (`context="Tutor"`) with in-chat cards (file-change lists, questionnaires, edit-options, validation, action, and hand-off) in `customContent`. The composer (`AiTutorComposer`) uses CADS `AiChatInput` with Check my work and the optional mode selector in `leftActions`. File chips are CADS `AiChatFileChip` in a row **above** the input field (not inside it). Web Lab 2 infers Help, Plan, or Build from the request (mode-neutral for students), then applies route/dev-panel capability gates. The dev panel can enable or disable Build, Plan, and Help independently and attach runner-scoped contract text to each enabled path. Help uses the same Socratic, hint-first communication style across policy presets.
...
- Create-file, create-folder, create-plan, and rename flows use CADS `Modal` (`CreateFileModal` for new files; `NameInputModal` for folder/plan/rename) capped at `maxWidth={500}`. Filename flexes; the type field is a fixed 8.5rem column so labels are not truncated. These overlays wrap `CadsLabProvider` because they mount beside `Lab2Shell`, not inside it.

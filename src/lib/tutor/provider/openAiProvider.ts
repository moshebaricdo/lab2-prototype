...
import type {
  ...
  TutorInstructionStepSatisfactionResponse,
  TutorPatchResponse,
  TutorRequestIntentResponse,
...
export interface TutorInstructionAnalysisProvider {
  requestInstructionAnalysis(
    messages: TutorChatMessage[],
  ): Promise<TutorInstructionAnalysisResponse | null>;
}

export interface TutorInstructionOptionSelectionProvider {
  requestInstructionOptionSelection(
    messages: TutorChatMessage[],
  ): Promise<TutorInstructionOptionSelectionResponse | null>;
}

export interface TutorInstructionStepSatisfactionProvider {
  requestInstructionStepSatisfaction(
    messages: TutorChatMessage[],
  ): Promise<TutorInstructionStepSatisfactionResponse | null>;
}
    ...
    TutorInstructionOptionSelectionProvider
{
  ...
  async requestInstructionAnalysis(messages: TutorChatMessage[]) {
    return requestChatCompletionJson<TutorInstructionAnalysisResponse>({
      messages,
      maxTokens: 1600,
      temperature: 0.2,
      logPrefix: "TutorInstructionAnalysis",
    });
  }

  async requestInstructionOptionSelection(messages: TutorChatMessage[]) {
    return requestChatCompletionJson<TutorInstructionOptionSelectionResponse>({
      messages,
      maxTokens: 200,
      temperature: 0,
      logPrefix: "TutorInstructionOptionSelection",
    });
  }

  async requestInstructionStepSatisfaction(messages: TutorChatMessage[]) {
    return requestChatCompletionJson<TutorInstructionStepSatisfactionResponse>({
      messages,
      maxTokens: 200,
      temperature: 0,
      logPrefix: "TutorInstructionStepSatisfaction",
    });
  }
}

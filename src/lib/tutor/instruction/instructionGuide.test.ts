...
describe("stripInstructionAuthoringMetadata", () => {
  it("removes tutor-guide blocks from student-facing markdown", () => {
    ...
    // The Feature Roulette instructions are a worksheet with no heading; the
    // strip should preserve the student-facing body verbatim.
    expect(stripped).toContain("Create a New Feature");
...
describe("buildInstructionGuide", () => {
  ...
  it("builds a linear guide for Feature Roulette curriculum instructions", () => {
    ...
    expect(guide.type).toBe("linear");
    if (guide.type !== "linear") throw new Error("Expected linear guide");
    // The worksheet uses bold numbered labels with no heading/intro, so the
    // derived overview/firstMove fall back to the first step label. The keyed
    // opening runner and the worksheet-stripping fallback produce nicer
    // student-facing copy at delivery time.
    expect(guide.overview).toBe("1: Create a New Feature");
    expect(guide.firstMove).toBe("1: Create a New Feature");
    expect(guide.steps.map((step) => step.title)).toEqual([
      "1: Create a New Feature",
      "2: Save with a Comment",
      "3: Revert as Needed",
    ]);
    expect(guide.steps.every((step) => step.intent === "explain")).toBe(true);
